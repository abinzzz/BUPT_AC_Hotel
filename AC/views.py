...
class RoomInfo:  # Room->字典
    ...
    def __init__(self, room):
        ...
        self.dic["fee"] = round(room.fee, 2)
        self.dic["room_id"] = room.room_id
        self.dic["mode"] = room.mode
...
def get_room_id(request):
    ...
    if s_id not in room_c.dic:
        ...
        print('change happened here: ', room_c.num)
...
def client_off(request):  # 开机之前的操作: 第一次访问客户端界面、开机
...
def login(request):
    ...
    if request.method=="GET":
      ...
      if username =='root' and password == '123':
        ...
        return redirect("http://10.129.197.107:8000/")
      else:
          #return HttpResponse("登录失败")
        return render(request,"login.html",{"error_msg":"用户名或密码错误"})


def guanli_login(request):

    if request.method=="GET":
        return render(request,"guanli_login.html")
      ...
      if not role:
          return render(request, "guanli_login.html", {"error_msg": "请输入你的身份"})

      if role == "reception" and username =='recp' and password == '111':
          #return HttpResponse("登录成功")
        return render(request, "reception.html")
      elif role == "manager" and username =='mana' and password == '222':
          return render(request, "report.html")
      elif role == "monitor" and username == 'moni' and password == '333':
          return render(request, "init.html")
      else:
          #return HttpResponse("登录失败")
        return render(request,"guanli_login.html",{"error_msg":"用户名或密码错误"})